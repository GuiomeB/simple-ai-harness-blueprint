...
This repository dogfoods L+ on itself (it now carries the real `.claude/` hooks and `/loop` workflow), with the headless runner left at manual-dispatch to keep the blast radius minimal.
